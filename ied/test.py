from ied.base import Fact, Relation, TestableRelation, Variable, Object, Rule
from ied.engine import InferenceEngine
from ied.relations import And, Or
from ied.geo import GeoObject, intersects
...
someone = Variable('someone')
aristotelesIsMortal = ie.evalExpression(mortal, aristoteles)
someoneIsMortal = ie.evalExpression(mortal, someone)
bothHumanAndMortal = ie.evalExpression(And, (human, X), (mortal, X))

assert aristotelesIsMortal == [{}]
assert someoneIsMortal == [{someone: alf}, {someone: aristoteles}]
assert bothHumanAndMortal == [{X: aristoteles}]
...
assert billsParents == [{M: anne, F: bill}]
assert daughters == [{X: doreen, M: anne, F: bill},
                     {X: eve, M: anne, F: bill}]
...
assert volkerIsMyBrothersFather == [{X: andreas}]
...
france = Country('France', Polygon([[0,0],[1,0],[1,1]]))
germany = Country('Germany', Polygon([[0,0],[1,1],[0,1]]))
rhine = River('Rhine', LineString([[-1,-1],[0,0],[1,1]]))
controls = Relation('controls')

someRiver = Variable('river', River)
someCountry = Variable('country', Country)
...
assert someoneControlsRhine == [{someCountry: germany}, {someCountry: france}]
...
assert dangerousVeis == [{V: vei3, L: lahar3}, {V: vei4, L: lahar4}]
assert allDangerousVeis == [{V: vei3, L: lahar3}, {V: vei4, L: lahar4}, {V: vei4, L: ashf4}]
